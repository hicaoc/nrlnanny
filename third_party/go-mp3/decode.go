...
func (d *Decoder) readFrame() error {
	...
	if err != nil {
		...
		// Skip the corrupt frame instead of aborting the whole stream, like
		// mpg123/ffmpeg do. The next frameheader.Read scans forward to the
		// next valid frame header, so decoding resynchronizes on its own.
		// (nrlnanny patch: fixes files that failed with e.g. "is_pos was
		// too big" even though other players handle them fine.)
		return nil
	}
	d.buf = append(d.buf, decodeFrameSafe(d.frame)...)
	return nil
}

// decodeFrameSafe decodes one frame, treating inconsistent frame data that
// would panic the decoder as a skipped (silent) frame instead of crashing
// the process.
func decodeFrameSafe(f *frame.Frame) (buf []byte) {
	defer func() {
		if recover() != nil {
			buf = nil
		}
